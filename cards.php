...
if (stripos($_SERVER['REQUEST_URI'], 'friends')) {
    // Query for liked Palz (Friends page)
    $sql = "SELECT palz.id, palz.name, palz.age, palz.birthday, palz.image_url, 
                   COUNT(user_likes.palz_id) AS like_count,
                   1 AS isLiked
            FROM palz
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="palz_likes">
                        <?php
                        $iconName = $row['isLiked'] ? 'heart' : 'heart-outline';
                        ?>

                        <span class="like-btn" data-palz-id="<?= $palz_id ?>"><ion-icon name="<?= $iconName; ?>"></ion-icon></span>
